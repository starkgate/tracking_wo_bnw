...
class Tracker:
	...
	def reid(self, blob, new_det_pos, new_det_scores):
		...
		if self.do_reid:
			new_det_features = self.reid_network.test_rois(blob, new_det_pos).data
	...
	def get_appearances(self, blob):
		...
		new_features = self.reid_network.test_rois(blob, self.get_pos()).data
		return new_features
	...
	def align(self, blob):
		...
		if self.im_index > 0:
			...
			# workaround for cv2 crash, empty inputMask and 1px gaussian blur
	...
	def step(self, blob):
		...
		if len(boxes) > 0:
			...
			inds = np.greater(scores, self.detection_person_thresh).nonzero()
		else:
			inds = torch.zeros(0).to(self.device)

		if len(inds) > 0:
			det_pos = torch.FloatTensor(boxes[inds]).to(self.device)
			det_scores = torch.FloatTensor(scores[inds]).to(self.device)
		else:
		...
		if len(self.tracks):
			...
			# regress
			person_scores = self.regress_tracks()
